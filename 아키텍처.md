...
- 기본 요청을 2회 모델 호출과 전체 60초 예산 안에서 처리한다.
...
- OCR.space 일본어 OCR
- Tavily 일본 작품·한국 정보 검색
- SerpApi Google Lens fallback
- Azure AI Foundry 최종 판정
- 이미지 원본 비영구 저장
...
| AI 연동 | 서버 전용 Azure Foundry REST gateway | 후보 비교·근거 검증·최종 판정 |
...
- 서버 전용 코드는 Node.js 실행 환경을 전제로 하며, 파일 시스템·환경변수·Azure Foundry 인증 정보는 서버 모듈에서만 접근한다.
- React 컴포넌트와 서버 모듈 사이에는 공유 타입만 전달하고, 서버 전용 gateway 타입이나 원시 Foundry 응답을 UI에 노출하지 않는다.
...
flowchart TD
		...
		R --> V[입력 검증·이미지 전처리]
		V --> P1[① Gemini 이미지 분석 + Google Search grounding\nOCR·단서·검색 결과]
		P1 --> P2[② Gemini 최종 판정\n일본 TOP 3·한국 제목·정발·근거]
		P2 --> O[응답 스키마 검증·결과 변환]
		O --> U
		R -.-> L[구조화 로그·메트릭]
		P1 -.-> L
		P2 -.-> L
...
### 5.1 ① 이미지 분석·검색: Gemini

입력:

- 전처리된 이미지 1~3장
- OCR·단서 추출 및 Google Search grounding 프롬프트
...
- 최종 TOP 3과 한국 정보를 확정하지 않음
- 검색 쿼리와 검색 결과 요약을 ②에 전달
...
- ①의 검증된 OCR·단서·Google Search 결과 JSON

처리:

- 일본 작품 후보 TOP 3을 선정한다.
- 한국 공식 제목·통용명·AI 참고 번역·정발 여부를 반환한다.
...
| ①이 `INSUFFICIENT` | `INSUFFICIENT` | 빈 배열 |
| ① 또는 ②가 외부 오류·검증 실패·timeout | `FAILED` | 빈 배열 |

② 응답 검증이 실패하면 ①의 원시 결과를 사용자에게 직접 반환하지 않는다. 오류 상태와 재시도 안내만 반환한다.

### 6.3 후보별 nullable 규칙

- `SUCCESS`: `koreanTitle`은 문자열이며 필드를 생략하지 않는다.
- `PARTIAL_SUCCESS`: ②가 후보를 반환했지만 일부 후보의 한국 정보가 `UNKNOWN`인 상태다.
