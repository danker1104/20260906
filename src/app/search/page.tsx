...
const idleStages = { imageAnalysis: 'SKIPPED', finalJudgment: 'SKIPPED' } as const;
type AnalysisStages = { imageAnalysis: StageStatus | 'IN_PROGRESS'; finalJudgment: StageStatus | 'IN_PROGRESS' };

export default function SearchPage() {
  const [files, setFiles] = useState<File[]>([]);
  const [result, setResult] = useState<IdentifyResponse | null>(null);
  const [stages, setStages] = useState<AnalysisStages>(idleStages);
  ...
  async function identify() {
    ...
    setError(null);
    setResult(null);
    setStages({ imageAnalysis: 'IN_PROGRESS', finalJudgment: 'SKIPPED' });
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><a className="brand" href="/?skipIntro=1#site-title">MangaFind</a><span className="topbar-note">Japanese manga search</span></header>
      ...
      <section className="hero" aria-labelledby="page-title"><div className="hero-copy"><span className="section-kicker">Screenshot to story</span><h1 id="page-title">이 만화, <em>뭐였지?</em></h1><p>SNS에서 발견한 일본 만화를 캡처 한 장으로 찾아보세요.</p></div><div className="hero-signal" aria-hidden="true"><span>01</span><span>image clue</span><span>→</span><span>verified title</span></div></section>
      {loading && <section className="analysis-section"><div className="section-heading"><div><span className="section-kicker">Live analysis</span><h2>단서를 이어 붙이고 있어요</h2></div></div><AnalysisProgress stages={stages} /></section>}
